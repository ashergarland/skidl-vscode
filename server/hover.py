...
def get_hover(
    ...
    """Return hover info for the given position, or None."""
    ...
    for pc in analysis.part_calls:
        ...
        if pc.symbol_span:
            ...
            if sl <= line_idx <= el and sc <= col <= ec:
                sym = index.get_symbol(pc.library, pc.symbol)
                if sym:
                    return _symbol_hover(sym, pc.symbol_span)
            ...
            if sl <= line_idx <= el and sc <= col <= ec:
                if index.symbol_lib_exists(pc.library):
                    ...
                    return Hover(
                        ...
                        range=_make_range(pc.library_span),
                    )
        ...
        if pc.footprint_span and pc.footprint and ":" in pc.footprint:
            ...
            if sl <= line_idx <= el and sc <= col <= ec:
                ...
                if fp:
                    return _footprint_hover(fp, pc.footprint_span)
...
def _symbol_hover(sym, span: tuple[int, int, int, int]) -> Hover:
    ...
    return Hover(
        ...
        range=_make_range(span),
    )


def _footprint_hover(fp, span: tuple[int, int, int, int]) -> Hover:
